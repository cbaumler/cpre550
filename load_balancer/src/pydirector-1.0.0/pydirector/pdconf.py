...
class PDGroup(object):
    __slots__ = [ 'name', 'scheduler', 'hosts', 'peers' ]
    ...
    def getPeerNamess(self):
        return self.peers.keys()

    def getPeers(self):
        return self.peers.values()

    def addPeer(self, name, ip):
        self.peers[name] = PDPeer(name, ip)
...
class PDService(object):
    ...
    def loadGroup(self, groupobj):
        ...
        cc = 0
        for c in groupobj.childNodes:
            if c.nodeName in ("#text", "#comment"): continue
            if c.nodeName not in ( u'host' u'peer' ):
                raise ConfigError, \
                    "expected 'host' or 'peer', got '%s'"%c.nodeName
            name = c.getAttribute('name')
            if not name: name = 'c.%s'%cc
            if c.nodeName == u'host':
                newgroup.addHost(name, c.getAttribute('ip'))
            if c.nodeName == u'peer':
                newgroup.addPeer(name, c.getAttribute('ip'))
            cc += 1
...
class PDConfig(object):
    __slots__ = [ 'services', 'admin', 'dom', 'rpcserver' ]

    def __init__(self, filename=None, xml=None):
        import pdlogging
        self.services = {}
        self.rpcserver = "localhost"
        self.admin = None
        ...
        for item in dom.childNodes:
            ...
            if item.nodeName not in ( u'service', u'admin', u'logging', u'rpcserver'):
                raise ConfigError, \
            ...
            elif item.nodeName == u'rpcserver':
                self.rpcserver = item.getAttribute('ip')

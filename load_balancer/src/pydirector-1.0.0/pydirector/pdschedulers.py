...
def createScheduler(groupConfig):
    ...
    elif schedulerName == "leastconnsdist":
        print "Using distributed scheduler algorithm"
        return LeastConnsDistributedScheduler(groupConfig)
    else:
...
class LeastConnsDistributedScheduler(BaseScheduler):
    """
        This scheduler takes into account the open connections
        of its peer load balancers.
    """
    schedulerName = "leastconnsDist"
    counter = 0

    #RPC function for retrieving peer load balancers' connection stats
    def getPeerConnections(self, ip):
        import xmlrpclib
        address = "http://%s/" % ip
        proxy = xmlrpclib.ServerProxy(address)
        return proxy.getConnectionData()

    #Function to determine the next host for making a connection
    def nextHost(self, client_addr):
        if not self.openconns.keys():
            return None
        #Create a list of hosts
        hosts = [ (x[1],x[0]) for x in self.openconns.items() ]
        distconns = [0] * len(hosts)
        disthosts = []
        #Loop through each peer load balancer
        for peer in self.peers:
            #Retrieve the peer's connection stats using RPC
            stats = self.getPeerConnections(peer)
            for s in stats:
                #Check each host ip for a matching peer host ip
                for idx, host in enumerate(hosts):
                    key = "%s:%s" % host[1]
                    #If a match is found, add the open connections for the 
                    #peer host to distconns[]
                    if s['open'][key] is not None:
                        distconns[idx] += s['open'][key]
        #Loop through the hosts and add the distributed host connections to 
        #this load balancer's host connections 
        for idx, host in enumerate(hosts):
            totalConns = host[0] + distconns[idx]
            disthosts.append((totalConns,host[1]))
        #Sort the hosts by number of connections and then hostname
        disthosts.sort()
        return disthosts[0][1]
